...
##### 進捗
 - 001：更新予定
 - 承認否認に関して：実装済、細かいデザインは決まっていないが明示させたい
 - 044：タブスタイル→ボタンへ
 - my box：後ろ白（優先度は低め、現在着手しているタスクが終わり次第）
 - 電子証明書登録のチェック：対応済み
 - phong：自分の画面＆担当以外の画面をチェック
 - 先方からまだ伝えきれていないタスクがくる

##### 今後
 - 従来通り画面作成はこちら
 - 今まで以上に入念にチェック
 - 試験環境変更（ip add変更）：後ほど先方から連絡が来る
 - 本日本番にもリリースあり（下記それに伴って影響ある画面）
   - 従業員詳細（支給項目）
 ...
 - しかし不満や利益がない
 - 今回外部から
 - JD作成：〜 １週間（VFJとやり取りを含め）※ iOS & BPM
   - コミュ力高いbpm or 技術よりbpm のようなことを里山さんに聞く
   - iosはどんなスキルが必要か要確認
 - JDの変更が必要（里山さん）か確認する（レビュー）
 - その後okであれば求人サイトまたはFBに投稿
 - 第一面接はスクリーニング（管理ファイル作成（特徴や名前をまとめる）
 - 第二面接(ios)は翻訳必要か要確認（里山さんへ）
 - 結果を応募者に通知→スオンさんへ
...
JD
 job：何対応するか（採用後何をするか）
 skill：そのまんまw 必要なこと
 benefit：VFJは違うので要確認（従来のJDを渡す）
 it viecの写真変更か要確認（した方が良い）
 上記里山さんに依頼
...
JD: iosのJDを里山さんに渡し
 - 変更があるかないか
 - ある場合は変更していただく
 - お疲れ様です。こちらVFAで使用しているBSEのJDです。こちら一度ご確認頂いて、追加内容や変更点があればこちらで修正し、外部に投稿します。
